...
- **TypeScript/JavaScript bindings** (`bindings/typescript/`, npm package `zigfitsio`):
  ...
  stack-argument layout bug (bun ≤1.3.14, unreported upstream). 132 tests mirror the pytest
...
- **TypeScript bindings — TS-native ergonomic layer** over the same astropy-shaped classes,
  ...
  - **Element-typed arrays + cutouts:** `FitsArray<T>.get/set` type as `bigint` for 64-bit
    ...
    tile-compressed image — read `.data`). 24 TS-specific tests on both runtimes; the public
    generics are type-checked via `tsconfig.test.json` (`npm run typecheck:tests`).
